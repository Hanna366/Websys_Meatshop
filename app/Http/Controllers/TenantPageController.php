...
class TenantPageController extends Controller
{
    ...
    public function storeUser(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'role' => array_filter(['required', Rule::in($availableRoles)]),
        ...
        try {
            ...
            $passwordWasAutoGenerated = false;
            if ($plainPassword === '') {
            ...
            // If password was auto-generated, email credentials to the user
            if (!empty($passwordWasAutoGenerated)) {
                try {
                    \Illuminate\Support\Facades\Mail::to($user->email)
                        ->queue(new \App\Mail\NewUserMail(
                            tenantName: $this->resolveTenant($request)?->business_name ?? 'Your Tenant',
                            userName: $user->name,
                            userEmail: $user->email,
                            generatedPassword: $plainPassword,
                        ));
                } catch (\Throwable $mailEx) {
                    // Swallow mail exceptions to avoid breaking the user creation workflow
                    \Log::warning('Failed to queue new user email', ['error' => $mailEx->getMessage(), 'email' => $user->email]);
                }
            }
    ...
    public function inventory(Request $request)
    {
        ...
        // Determine permissions for current user (supports RBAC or Spatie)
        $canViewInventory = false;
        $canManageInventory = false;

        try {
            if ($user) {
                // Use RbacService for legacy mapping and Spatie when available
                $canViewInventory = \App\Services\RbacService::userHasPermission($user, 'inventory.view')
                    || (method_exists($user, 'can') && $user->can('inventory.view'));

                $canManageInventory = \App\Services\RbacService::userHasPermission($user, 'inventory.add_stock')
                    || (method_exists($user, 'can') && $user->can('inventory.add_stock'))
                    || \App\Services\RbacService::userHasPermission($user, 'can_manage_inventory');
            }
        } catch (\Throwable $e) {
