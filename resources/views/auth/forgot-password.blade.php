...
<main class="mx-auto flex min-h-screen w-full max-w-6xl items-center justify-center p-4 sm:p-6">
    <section class="w-full max-w-md rounded-3xl border p-6 shadow-2xl backdrop-blur sm:p-8" style="border-color: var(--card-border); background: rgba(8, 2, 2, 0.74); box-shadow: 0 35px 90px rgba(0, 0, 0, 0.45);">
        <div class="mb-6 text-center">
            <h1 class="heading-font text-2xl font-semibold" style="color: var(--text);">Forgot Password</h1>
            <p class="mt-1 text-sm" style="color: var(--muted);">Enter your email and we will generate a reset link.</p>
        </div>
        ...
        @if(session('reset_link'))
            <div class="mb-4 rounded-xl border border-cyan-400/45 bg-cyan-500/10 px-4 py-3 text-sm text-cyan-100">
                <div class="mb-2 font-semibold">Development reset link:</div>
                <a href="{{ session('reset_link') }}" class="break-all text-cyan-200 underline decoration-cyan-300/60 underline-offset-2 hover:text-cyan-100">{{ session('reset_link') }}</a>
            </div>
        @endif
        ...
        <form method="POST" action="/forgot-password" class="space-y-4">
            ...
            <div>
                ...
                <input type="email" id="email" name="email" value="{{ old('email') }}" placeholder="you@meatshop.com" class="h-11 w-full rounded-lg border bg-white/5 px-3 text-sm text-rose-50 outline-none transition placeholder:text-rose-200/55 focus:border-rose-400 focus:ring-2 focus:ring-rose-500/35" style="border-color: rgba(255, 255, 255, 0.28);" required>
            </div>

            <button type="submit" class="inline-flex h-11 w-full items-center justify-center rounded-lg bg-gradient-to-r from-rose-800 to-rose-500 px-4 text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-rose-500 focus:ring-offset-2 focus:ring-offset-rose-950/40" style="box-shadow: 0 10px 28px rgba(246, 52, 112, 0.28);">Send Reset Link</button>
        </form>
