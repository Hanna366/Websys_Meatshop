...
<?php $__env->startSection('page_subtitle', 'Manage account details, security preferences, and subscription status'); ?>
...
<section class="grid gap-6 lg:grid-cols-12">
    <aside class="space-y-6 lg:col-span-4">
        <section class="rounded-3xl border border-white/70 bg-white/90 p-6 text-center shadow-card">
            <img src="https://ui-avatars.com/api/?name=<?php echo e(urlencode(session('user.name', 'Owner User'))); ?>&background=1e40af&color=fff" alt="Profile Avatar" class="mx-auto mb-4 h-24 w-24 rounded-full border-4 border-white shadow">
            <h2 class="heading-font text-xl font-semibold text-slate-900"><?php echo e(session('user.name', 'Owner User')); ?></h2>
            <p class="text-sm text-slate-500"><?php echo e(session('user.email', 'owner@meatshop.com')); ?></p>
            <span class="mt-3 inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold <?php echo e(session('user.plan') == 'Premium' ? 'bg-rose-100 text-rose-700' : (session('user.plan') == 'Standard' ? 'bg-amber-100 text-amber-700' : 'bg-indigo-100 text-indigo-700')); ?>">
                <i data-lucide="crown" class="h-3.5 w-3.5"></i>
                <?php echo e(session('user.plan', 'Basic')); ?> Plan
            </span>
        </section>
    ...
    <div class="space-y-6 lg:col-span-8">
        <section class="rounded-3xl border border-white/70 bg-white/90 p-5 shadow-card sm:p-6">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <div><p class="text-xs uppercase tracking-wide text-slate-500">Full Name</p><p class="mt-1 font-medium text-slate-800"><?php echo e(session('user.name', 'Owner User')); ?></p></div>
                <div><p class="text-xs uppercase tracking-wide text-slate-500">Email</p><p class="mt-1 font-medium text-slate-800"><?php echo e(session('user.email', 'owner@meatshop.com')); ?></p></div>
                ...
                <div><p class="text-xs uppercase tracking-wide text-slate-500">Role</p><p class="mt-1 font-medium text-slate-800"><?php echo e($profileRoleLabel); ?></p></div>
        ...
        <section class="rounded-3xl border border-white/70 bg-white/90 p-5 shadow-card sm:p-6">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <div><p class="text-xs uppercase tracking-wide text-slate-500">Last Login</p><p class="mt-1 font-medium text-slate-800"><?php echo e(now()->format('F j, Y g:i A')); ?></p></div>
                <div>
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.tenant', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\OWNER\Documents\webs\meatshop\resources\views/profile.blade.php ENDPATH**/ ?>