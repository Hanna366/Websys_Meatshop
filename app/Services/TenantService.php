...
class TenantService
{
     ...
     */
    public static function createTenant(array $data, bool $autoProvision = true): Tenant
    {
        ...
        try {
            // If auto-provisioning is requested, attempt to create the tenant database
            // and run tenant migrations/seeds and create initial admin user. Otherwise
            // only create the central tenant record and domain so the tenant can be
            // reviewed/approved by a central admin before provisioning.
            ...
            $tenant = DB::connection($defaultConnection)->transaction(function () use (
                ...
                $tenant = Tenant::create([
                    ...
                    // When not auto-provisioning, default to 'pending' so central
                    // admins can approve the tenant before provisioning.
                    'status' => $data['status'] ?? ($autoProvision ? 'active' : 'pending'),
                    'payment_status' => $data['payment_status'] ?? 'paid',
                    'suspended_message' => $data['suspended_message'] ?? ($data['disabled_message'] ?? 'Please contact your administrator.'),
                    'disabled_message' => $data['disabled_message'] ?? ($data['suspended_message'] ?? 'Please contact your administrator.'),
                    'domain' => $domain,
                    // Only populate DB credentials when auto-provisioning.
                    'db_name' => $autoProvision ? $dbName : null,
                    'db_username' => $autoProvision ? $dbUsername : null,
                    'db_password' => $autoProvision ? encrypt($dbPasswordPlain) : null,
                    'plan' => $plan,
            ...
            // If auto-provisioning was requested, continue with provisioning steps
            // which include running tenant migrations, creating the tenant admin
            // user in the tenant database, and sending the onboarding email.
            if ($autoProvision) {
    ...
    public static function updateTenantSubscription(string $tenantId, array $payload): Tenant
    {
        ...
        if ($status === 'expired') {
            ...
            if (empty($tenant->disabled_message) && !empty($tenant->suspended_message)) {
                $tenant->disabled_message = $tenant->suspended_message;
            }
            if (empty($tenant->disabled_message)) {
                $tenant->disabled_message = 'Subscription expired. Please renew to restore access.';
            }
        }
    ...
    /**
     * Provision an existing pending tenant: create DB, run migrations/seeders,
     * create admin user on tenant DB, and send onboarding email.
     */
    public static function provisionTenant(Tenant $tenant, bool $returnPassword = false)
    {
        ...
        $dbName = self::generateDatabaseName($tenantId);
        $dbUsername = 'tenant_' . substr(sha1($tenantId), 0, 10);
        $dbPasswordPlain = Str::random(24);
        ...
        try {
            ...
            // Update tenant central record with DB credentials and set active.
            $tenant->db_name = $dbName;
            $tenant->db_username = $dbUsername;
            $tenant->db_password = encrypt($dbPasswordPlain);
            $tenant->status = 'active';
            $tenant->payment_status = $tenant->payment_status ?? 'paid';

            // Ensure subscription snapshot reflects provisioning (active)
            $existingSubscription = is_array($tenant->subscription) ? $tenant->subscription : [];
            $now = now();
            $periodStart = $existingSubscription['current_period_start'] ?? $now->toDateString();
            $periodEnd = $existingSubscription['current_period_end'] ?? $now->copy()->addMonth()->toDateString();

            $tenant->subscription = array_merge($existingSubscription, [
                'plan' => $existingSubscription['plan'] ?? $tenant->plan ?? 'basic',
                'status' => 'active',
            ...
            // Create initial admin user on tenant database with a secure generated password
            $isAutoGeneratedPassword = true;
            $adminPassword = self::generateSecurePassword();
            $adminEmail = $tenant->admin_email ?? $tenant->business_email ?? null;
            $adminName = $tenant->admin_name ?? $tenant->business_name ?? 'Tenant Admin';
            $adminUsername = Str::slug($adminName, '_');

            // Create or update the initial admin user on tenant database
            tenancy()->initialize($tenant);
            app(PermissionRegistrar::class)->forgetCachedPermissions();

            $existingUser = \App\Models\User::on('tenant')
                ->where('email', $adminEmail)
                ->orWhere('username', $adminUsername)
                ->first();

            if ($existingUser) {
                $existingUser->password = \Illuminate\Support\Facades\Hash::make($adminPassword);
            ...
            // Assign Owner role if present
            $role = \Spatie\Permission\Models\Role::on('tenant')->where('name', 'Owner')->first();
            if ($role && isset($adminUser)) {
                $adminUser->syncRoles([$role]);
            }

            tenancy()->end();

            $sent = self::sendTenantOnboardingEmail(
                $tenant,
                (string) $adminEmail,
                (string) $adminName,
                $isAutoGeneratedPassword ? $adminPassword : null
            ...
            // Persist only real tenant fields; do not write ephemeral flags or plaintext passwords.
            $tenant->save();

            if ($returnPassword && $isAutoGeneratedPassword) {
                return [
                    'tenant' => $tenant,
            ...
            // Attempt cleanup of any partially created DB artifacts.
            self::cleanupFailedProvisioning($tenant, $dbName, $databaseExistedBefore, $defaultConnection, $driver);
            throw $e;
        }
    }
