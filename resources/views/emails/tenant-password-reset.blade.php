...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #1f2937; margin: 0; padding: 0; background: #f3f4f6;">
    <div style="max-width: 560px; margin: 24px auto; background: #ffffff; border: 1px solid #e5e7eb; border-radius: 10px; overflow: hidden;">
        <div style="background: #047857; color: #ffffff; padding: 18px 22px;">
            <h2 style="margin: 0; font-size: 20px;">Tenant Account</h2>
        </div>

        <div style="padding: 22px;">
            <p style="margin-top: 0;">Hi {{ $userName ?? ($user->name ?? $user->email ?? $email) }},</p>
            <p>We received a request to reset your password for your tenant account. Click the button below to continue.</p>

            <p style="margin: 22px 0;">
                <a href="{{ $resetUrl }}" style="display: inline-block; padding: 10px 16px; background: #047857; color: #ffffff; text-decoration: none; border-radius: 6px;">Reset Tenant Password</a>
            </p>
