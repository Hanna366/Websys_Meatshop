        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
                ...
                <p class="mb-0 text-sm text-slate-500">
                    <?php echo e($centralBillingMode ? 'Monitor tenant payment status and billing overview.' : 'Manage subscription invoices and payment history.'); ?>
    ...
    <section class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
        <article class="rounded-2xl border border-slate-200/70 bg-white p-5 shadow-card transition hover:-translate-y-0.5 hover:shadow-xl">
            <div class="flex items-start justify-between">
                <div>
                    <p class="mb-1 text-sm font-medium text-slate-500"><?php echo e($centralBillingMode ? 'Managed Tenants' : 'Current Plan'); ?></p>
                    <h3 class="heading-font text-2xl font-semibold text-slate-900"><?php echo e($centralBillingMode ? number_format((int) $plan) : ucfirst((string) $plan)); ?></h3>
                </div>
        ...
        <article class="rounded-2xl border border-slate-200/70 bg-white p-5 shadow-card transition hover:-translate-y-0.5 hover:shadow-xl">
            <div class="flex items-start justify-between">
                <div>
                    <p class="mb-1 text-sm font-medium text-slate-500"><?php echo e($centralBillingMode ? 'Estimated MRR' : 'Monthly Cost'); ?></p>
        ...
        <article class="rounded-2xl border border-slate-200/70 bg-white p-5 shadow-card transition hover:-translate-y-0.5 hover:shadow-xl">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <?php if($centralBillingMode): ?>
                        <p class="mt-2 text-xs text-slate-500">Paid: <?php echo e((int) ($subscription['paid_tenants'] ?? 0)); ?> | Unpaid/Overdue: <?php echo e((int) ($subscription['unpaid_tenants'] ?? 0)); ?></p>
                    <?php endif; ?>
                </div>
    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200/70 bg-white shadow-card">
        ...
        <?php if(empty($billingHistory)): ?>
            <div class="px-6 py-12 text-center">
                ...
                <h4 class="heading-font mb-1 text-lg font-semibold text-slate-900"><?php echo e($centralBillingMode ? 'No tenant billing records yet' : 'No invoices yet'); ?></h4>
                <p class="mb-0 text-sm text-slate-500"><?php echo e($centralBillingMode ? 'Tenant payment records will appear here as tenants are updated.' : 'Invoices will appear here once your first billing cycle is processed.'); ?></p>
            </div>
        <?php else: ?>
                    ...
                    <tbody class="divide-y divide-slate-100 text-slate-700" id="billingTableBody">
                        <?php $__currentLoopData = $billingHistory; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $entry): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            <tr class="billing-row transition duration-150 hover:bg-indigo-50/40">
                                ...
                                <?php if($centralBillingMode): ?>
                                    <td class="px-5 py-4 text-slate-700"><?php echo e($entry['tenant_name'] ?? '-'); ?></td>
                                <?php endif; ?>
