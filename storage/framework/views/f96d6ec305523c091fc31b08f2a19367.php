...
<section class="space-y-6">
    <?php if(!$isUnlimitedProducts): ?>
        <div class="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
            Product cap: <span class="font-semibold"><?php echo e((int) $maxProducts); ?></span>. Current count: <span class="font-semibold"><?php echo e($currentProductCount); ?></span>.
        </div>
    <?php endif; ?>
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <article class="rounded-3xl border border-white/70 bg-white/90 p-5 shadow-card">
            ...
            <p class="mt-2 text-2xl font-bold text-slate-900"><?php echo e(number_format((int) data_get($productStats ?? [], 'total_products', 0))); ?></p>
        ...
        <article class="rounded-3xl border border-white/70 bg-white/90 p-5 shadow-card">
            ...
            <p class="mt-2 text-2xl font-bold text-amber-600"><?php echo e(number_format((int) data_get($productStats ?? [], 'low_stock_products', 0))); ?></p>
        ...
        <article class="rounded-3xl border border-white/70 bg-white/90 p-5 shadow-card">
            ...
            <p class="mt-2 text-2xl font-bold text-emerald-600"><?php echo e(data_get($productStats ?? [], 'average_margin') !== null ? number_format((float) data_get($productStats, 'average_margin', 0), 1) . '%' : 'N/A'); ?></p>
        ...
        <article class="rounded-3xl border border-white/70 bg-white/90 p-5 shadow-card">
            ...
            ?>
            <p class="mt-2 text-2xl font-bold text-indigo-600"><?php echo e($catalogHealth); ?></p>
            <p class="mt-1 text-xs text-slate-500"><?php echo e(number_format($outOfStock)); ?> out-of-stock item(s)</p>
        </article>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200 text-sm">
                ...
                <tbody id="productsTable" class="divide-y divide-slate-100 text-slate-700">
                    <?php $__empty_1 = true; $__currentLoopData = ($pageProducts ?? collect()); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <?php
                        ...
                        ?>
                        <tr>
                            <td class="px-3 py-3 font-medium"><?php echo e($product->name); ?></td>
                            <td class="px-3 py-3"><?php echo e(ucfirst((string) ($product->category ?? 'Uncategorized'))); ?></td>
                            <td class="px-3 py-3 font-semibold">PHP <?php echo e(number_format($price, 2)); ?></td>
                            <td class="px-3 py-3"><?php echo e(number_format($stock, 2)); ?> <?php echo e(data_get($product->inventory, 'unit_of_measure', 'kg')); ?></td>
                            <td class="px-3 py-3"><span class="rounded-full px-2.5 py-1 text-xs font-semibold <?php echo e($statusClass); ?>"><?php echo e($statusLabel); ?></span></td>
                            <td class="px-3 py-3 text-right">
                                <div class="inline-flex gap-2">
                                    <button onclick="editProduct('<?php echo e(addslashes((string) $product->name)); ?>')" class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-600 hover:bg-slate-50">Edit</button>
                                    <button onclick="deleteProduct('<?php echo e(addslashes((string) $product->name)); ?>')" class="rounded-lg border border-rose-200 px-3 py-1.5 text-xs font-semibold text-rose-600 hover:bg-rose-50">Delete</button>
                                </div>
                            </td>
                        </tr>
