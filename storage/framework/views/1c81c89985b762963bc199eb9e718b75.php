...
<section class="grid gap-6 max-w-7xl mx-auto lg:grid-cols-4">
    ...
    <div class="lg:col-span-3 space-y-6">
        ...
        <section id="users" class="rounded-3xl border border-white/70 bg-white/90 p-5 shadow-card sm:p-6">
            ...
            <form method="POST" action="<?php echo e($userStoreAction); ?>" class="mb-6 grid gap-3 rounded-2xl border border-slate-200 bg-slate-50/60 p-4 sm:grid-cols-2 lg:grid-cols-3">
                <?php echo csrf_field(); ?>
                <div class="sm:col-span-1 lg:col-span-1">
                    ...
                    <input id="add_user_name" name="name" value="<?php echo e(old('name')); ?>" class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm" placeholder="Juan Dela Cruz" required>
...
unset($__errorArgs, $__bag); ?>
                ...
                <div class="sm:col-span-1 lg:col-span-1">
                    ...
                    <input id="add_user_email" type="email" name="email" value="<?php echo e(old('email')); ?>" class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm" placeholder="cashier@shop.com" required>
...
unset($__errorArgs, $__bag); ?>
                ...
                <div class="sm:col-span-1 lg:col-span-1">
                    ...
                    <input id="add_user_username" name="username" value="<?php echo e(old('username')); ?>" class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm" placeholder="auto-generated if empty">
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        <?php $__empty_1 = true; $__currentLoopData = ($tenantUsers ?? []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tenantUser): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            ...
                            <tr class="hover:bg-slate-50">
                                <td class="px-4 py-3 font-medium text-slate-900"><?php echo e($tenantUser->username); ?></td>
                                <td class="px-4 py-3 text-slate-700"><?php echo e($tenantUser->email); ?></td>
                                <td class="px-4 py-3">
                                    <span class="inline-flex rounded-full px-2 py-1 text-xs font-semibold <?php echo e($roleClasses); ?>">
                                        <?php echo e($roleLabel); ?>

                                    </span>
                                </td>
                                <td class="px-4 py-3">
                                    <span class="inline-flex rounded-full px-2 py-1 text-xs font-semibold <?php echo e($statusClasses); ?>">
                                        <?php echo e(ucfirst($status)); ?>

                                    </span>
                                </td>
                                <td class="px-4 py-3 text-right">
                                    <?php if(in_array(strtolower((string) $roleLabel), ['owner', 'administrator'], true)): ?>
                                        <span class="inline-flex rounded-full px-2 py-1 text-xs font-medium text-slate-500 bg-slate-100">Protected</span>
                                    <?php else: ?>
                                        <div class="flex flex-col gap-2 items-end">
                                            <button onclick="toggleEditForm('edit-<?php echo e($tenantUser->id); ?>')" class="rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50 transition-colors">
                                                Edit
                                            </button>
                                            
                                            <div id="edit-<?php echo e($tenantUser->id); ?>" class="hidden w-full mt-2 p-3 bg-slate-50 rounded-lg border border-slate-200">
                                                <?php
                                                ...
                                                ?>
                                                <form method="POST" action="<?php echo e(\Illuminate\Support\Facades\Route::has('tenant.users.update') ? route('tenant.users.update', $tenantUser->id) : url('/settings/users/' . $tenantUser->id)); ?>" class="space-y-3">
                                                    <?php echo csrf_field(); ?>
                                                    <?php echo method_field('PUT'); ?>
                                                    <div class="grid grid-cols-1 gap-2">
                                                        <input type="text" name="name" value="<?php echo e($tenantUser->name); ?>" class="w-full rounded-lg border border-slate-200 px-3 py-2 text-xs" placeholder="Name" required>
                                                        <input type="text" name="username" value="<?php echo e($tenantUser->username); ?>" class="w-full rounded-lg border border-slate-200 px-3 py-2 text-xs" placeholder="Username" required>
                                                        <input type="email" name="email" value="<?php echo e($tenantUser->email); ?>" class="w-full rounded-lg border border-slate-200 px-3 py-2 text-xs" placeholder="Email" required>
                                                    ...
                                                    <div class="flex gap-2 justify-end">
                                                        <button type="button" onclick="toggleEditForm('edit-<?php echo e($tenantUser->id); ?>')" class="rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50 transition-colors">
                                                            Cancel
                                                        </button>
                                            ...
                                            ?>
                                            <form method="POST" action="<?php echo e($userStatusAction); ?>" class="inline">
                                                <?php echo csrf_field(); ?>
                                                <?php echo method_field('PATCH'); ?>
                                                <button type="submit" class="rounded-lg border px-3 py-1.5 text-xs font-medium transition-colors <?php echo e($status === 'active' ? 'border-rose-200 bg-rose-50 text-rose-700 hover:bg-rose-100' : 'border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100'); ?>">
